...
object StreamWithSchemaValidation {
  private val listOfNameAndAgeOfPersons =
    withFile(resourceName)(file => ZIO.attempt(getNameAndAgeOfPerson(file)))
  private val listOfEmailOfPersons =
    withFile(resourceName)(file => ZIO.attempt(getEmailOfPerson(file)))
  ...
  def getTotalNumberofRecords: ZIO[Any, Throwable, Int] =
    streamOfPersonWithNameAndAge.runCount
      .map(_.toInt)
  ...
  def listOfValidEmail = {
    for {
      list <- streamOfPersonWithEmail.runFold(List.empty[String])(
        (list, res) => {
          res match {
            case Left(_)     => list
...
object ExamplesSpec extends ZIOSpecDefault {
  ...
  def spec: Spec[Any, Throwable] = suite("ExampleSpec")(
    test("Total number of people is 31") {
      assertZIO(StreamWithSchemaValidation.getTotalNumberofRecords)(
        Assertion.equalTo(31)
      )
    },
    ...
    test(
      "First name in the stream of persons Above 18 is Michela Rizzo-Traetta"
    ) {
      for {
        persons <- StreamWithSchemaValidation.getFirstPersonAbove18
      } yield assertTrue(
        persons.head == PersonsData.firstFiveNamesOfPeopleAbove18.head
      )
    },
    test("First five names of persons above 18") {
      for {
        ...
        persons.take(5) == PersonsData.firstFiveNamesOfPeopleAbove18
      )
    },
    ...
    test(
      "Total number of valid emails ( valid emails must have numbers before the '@'"
    ) {
      for {
    ...
    test("valid emails list must match the one provided") {
      for {
        ...
        emails == PersonsData.listOfValidEmails,
        emails.size == 9
      )
    }
  )
